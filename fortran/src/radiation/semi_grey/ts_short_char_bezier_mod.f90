...
!  real(dp), dimension(nmu), parameter :: uarr = (/0.5_dp/)
...
contains
  ...
  subroutine ts_short_char_bezier(Bezier, surf, nlay, nlev, Ts, Tl, pl, pe,&
    ...
    real(dp), intent(in) :: Ts, AB, mu_s!, sw_a_surf, lw_a_surf
    real(dp), dimension(nlay), intent(in) :: Tl, pl, q
...
!real(dp), dimension(nlay), intent(in) :: sw_a, sw_g
    ...
    ! HII hack
    real(dp), dimension(nlev) :: tau_Ve, tau_IRe, mu_z
    real(dp), dimension(nlay) :: sw_a, sw_g
...
!    do i=2,nlay+1
!       tau_Ire(i) = tau_ire(i-1) + (kappa_lw_h2o*q(i-1) + k2_lw_h2o*(pl(i-1)/1.e5)*q(i-1)**2 + kappa_lw*(1-q(i-1)))*&
!            (pe(i) - pe(i-1))/grav
!       tau_Ve(i) = tau_Ve(i-1) + (kappa_sw_h2o*q(i-1) + k2_sw_h2o*(pl(i-1)/1.e5)*q(i-1)**2 + kappa_sw*(1-q(i-1)))*&
!            (pe(i) - pe(i-1))/grav
!    enddo
    ...
    if (surf .eqv. .True.) then
      ...
      be_int = Fint/pi ! Integrated planck function intensity for internal temperature
    end if
...
! No zenith correction, use regular method
         ...
         sw_down(:) = Finc * exp(-tau_Ve(:)/mu_z(nlev))
      else
...
end module ts_short_char_bezier_mod

module band_grey_mod
  ...
  use params,         only: dp, nf, Finc, Fint, sb, grav, opacity_dir, invert_grid, rdgas
...
contains
  ...
  subroutine calc_tau(P, Pe, T, Te, delP, qwat)
    ...
    real(dp) :: q_spec(6) ! Assume constant species for now, order:
    ! H2, H2O, CH4, NH3, CO2, CO
    npz  = size(T)

    mu = 18.* 5.67e-2 + 16.*3.70e-2 + 17*8.28e-4 + 44*8.84e-4 + 4.17e-3*28 &
         + (1 - 5.67e-2 - 3.70e-2 - 8.28e-4 - 8.84e-4 - 4.17e-3)*2.

    q_spec(2) = 18./mu * 5.67e-2
    q_spec(3) = 16./mu * 3.70e-2
    q_spec(4) = 17./mu * 8.28e-4
    q_spec(5) = 44./mu * 8.84e-4
    q_spec(6) = 28./mu * 4.17e-3
...
!!$       call linear_log_interp(pe(npz+1), P(npz-1), P(npz), T(npz-1), T(npz), qe(npz+1))
       ...
       do b=1,nB
          call bilinear_interp( tref_h2o, pref_h2o, h2o_kap(:,:,b), T1, p1, kp(2))
          call bilinear_interp( tref_ch4, pref_ch4, ch4_kap(:,:,b), T1, p1, kp(3))
          call bilinear_interp( tref_nh3, pref_nh3, nh3_kap(:,:,b), T1, p1, kp(4))
          call bilinear_interp( tref_co2, pref_co2, co2_kap(:,:,b), T1, p1, kp(5))
          call bilinear_interp( tref_co, pref_co, co_kap(:,:,b), T1, p1, kp(6))

          call lin_interp(tref_h2h2, h2h2_cia(:,b), T1, kp(1))
          call lin_interp(tref_h2oh2o, h2oh2o_cia(:,b), T1, kp(7))
          
          !call bilinear_interp( tref_h2h2, pref_h2h2, h2h2_cia(:,b), T1, p1, kp(1))
          !call bilinear_interp( tref_h2oh2o, pref_h2oh2o, h2oh2o_cia(:,:,b), T1, p1, kp(7))
...
!          kp2 = kp2*p2/T2/rdgas
          ...
          enddo
          dtau_c(b,1) = dtau_c(b,1) + delp(1)/grav * (q_spec(1)**2.*kp(1) + q_spec(2)**2.*kp(7)) &
                                                   * p1/R/T1
          ...
          call bilinear_interp( tref_h2o, pref_h2o, h2o_kap(:,:,b), T2, p2, kp(2))
          call bilinear_interp( tref_ch4, pref_ch4, ch4_kap(:,:,b), T2, p2, kp(3))
          call bilinear_interp( tref_nh3, pref_nh3, nh3_kap(:,:,b), T2, p2, kp(4))
          call bilinear_interp( tref_co2, pref_co2, co2_kap(:,:,b), T2, p2, kp(5))
          call bilinear_interp( tref_co, pref_co, co_kap(:,:,b), T2, p2, kp(6))
          ...
          enddo
          dtau_c(b,nf+1) = dtau_c(b,nf+1) + delp(nf+1)/grav * (q_spec(1)**2.*kp(1) + q_spec(2)**2.*kp(7))&
               * p2/R/T2

          !dtau_c(b,nf+1) = (kp1*q2 + kp2*(1-q2)) * delp(nf+1)/grav
          
          do k=2,npz
             call bilinear_interp( tref_h2o, pref_h2o, h2o_kap(:,:,b), Te(k), Pe(k), kp(2))
             call bilinear_interp( tref_ch4, pref_ch4, ch4_kap(:,:,b), Te(k), Pe(k), kp(3))
             call bilinear_interp( tref_nh3, pref_nh3, nh3_kap(:,:,b), Te(k), Pe(k), kp(4))
             call bilinear_interp( tref_co2, pref_co2, co2_kap(:,:,b), Te(k), Pe(k), kp(5))
             call bilinear_interp( tref_co, pref_co, co_kap(:,:,b), Te(k), Pe(k), kp(6))
             ...
             enddo
             dtau_c(b,k) = dtau_c(b,k) + delp(k)/grav * (q_spec(1)**2.*kp(1) + q_spec(2)**2.*kp(7))&
                                                       * Pe(k)/R/Te(k)
...
!!$             write(*,*) q_spec(5)*kp(5), q_spec(6)*kp(6), q_spec(2)**2.*kp(7)*Pe(k)/R/Te(k)
    ...
    do b=1, nB
       ...
       do  k=1,npz
          ...
          call bilinear_interp( tref_h2o, pref_h2o, h2o_kap(:,:,b), T(k), P(k), kp(2))
          call bilinear_interp( tref_ch4, pref_ch4, ch4_kap(:,:,b), T(k), P(k), kp(3))
          call bilinear_interp( tref_nh3, pref_nh3, nh3_kap(:,:,b), T(k), P(k), kp(4))
          call bilinear_interp( tref_co2, pref_co2, co2_kap(:,:,b), T(k), P(k), kp(5))
          call bilinear_interp( tref_co, pref_co, co_kap(:,:,b), T(k), P(k), kp(6))
...
!!$          endif
          ...
          dtau_u(b,k) = 0.
             ...
             dtau_u(b,k) = dtau_u(b,k) + delp(k)/grav * (q_spec(1)**2.*kp(1) + q_spec(2)**2.*kp(7))&
                                                       * P(k)/R/T(k)
             
            !dtau_u(b,k) = (kp1*qwat(k) + kp2*(1-qwat(k)))*delP(k)/grav
          if (dtau_u(b,k) .lt. 0) then
             write(*,*) 'SUSPICIOUS DTAU', Te(k), Pe(k), kp1, kp2, delp(k)
             stop
          endif
          
       enddo
!       write(*,*) b, maxval(kp(2:6)*q_spec(2:6)), maxloc(kp(2:6)*q_spec(2:6)), kp(1)*q_spec(1)**2., kp(7)*q_spec(2)**2.
    enddo
...
!    real(dp), allocatable :: utau_banded(:,:)
    ...
    real(dp) :: albedo = 1. ! Xianyu hack for internal temp
    real(dp) :: btemp = 1000
    ...
    allocate(plnk(nB))
!    allocate(utau_banded(nB, npz+1))
    plnk = (/.true., .true., .true., .true., .true., .true., .true., .true., .true., .true./)
    btemp = (Fint/sb)**0.25
    !write(*,*) btemp
    ! BTEMP CHANGED
...
!!$          enddo
!!$          write(*,*) '--------------------------------'temp2(npz+1)
          call twostr(albedo, btemp, deltam, dtau_c(n,:), fbeam(n), fisot, gg, header, ierror, &
...
!!$    write(*,*) 'COMP DTAU :, '
!!$    do k=1,npz
!!$       write(*,*) dtau_c(1,k), dtau_c(2,k), dtau_c(3,k), dtau_c(4,k), dtau_c(5,k)
!!$    enddo
!!$    
!!$    write(*,*) '-----------------'
!!$

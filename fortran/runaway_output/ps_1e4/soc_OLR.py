...
#    nctools.ncout3d('profile.h2o',0,0,pres_list,q_mr_list,'h2o',longname="h2o",units='PPMM')
    ...
    spec = params.spec_sw
    # Run Socrates:
    command = ("Cl_run_cdf -B", basename, "-s", spec, "-R 1 300 -ch 300 -S -g 8 0 -u -C 5 -t 15 -r")
...
#    nctools.ncout3d('profile.h2o',0,0,pres_list,q_mr_list,'h2o',longname="h2o",units='PPMM')
    ...
    spec = params.spec
    # Run Socrates:
    command = ("Cl_run_cdf -B", basename, "-s", spec, "-R 1 300 -ch 300 -I -g 8 0 -u -C 5 -t 15")
    command = " ".join(command)
